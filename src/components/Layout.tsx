...
import {
  LogOut, Bell, MoreHorizontal, X, Search,
  Loader2, RefreshCw,
} from 'lucide-react';
...
import { getMobileMoreNav, getMobilePrimaryNav } from '../config/navigation';
...
export default function Layout() {
  ...
  const primaryNav = getMobilePrimaryNav(currentUser?.role);
  const moreNav = getMobileMoreNav(currentUser?.role);

  return (
    <div className="flex min-h-screen bg-obsidian-950">
      <Sidebar />
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="safe-area-top glass-header border-b border-gold-500/20 flex items-center justify-between px-4 md:px-6 shrink-0 shadow-card sticky top-0 z-20"
          style={{ minHeight: 'calc(3.5rem + env(safe-area-inset-top, 0px))' }}
        >
          <button className="flex items-center gap-2.5 md:cursor-default text-left" onClick={() => setShowProfile(true)}>
            ...
            <div>
              <h2 className="text-white font-semibold text-sm md:text-base leading-none">{currentUser?.name ?? 'AutoDream'}</h2>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-obsidian-400/50 bg-obsidian-700/40 text-gray-500 hover:text-gray-300 hover:border-obsidian-400/80 transition-colors text-xs"
            >
              <Search size={13} />
              <span>Search</span>
              <span className="ml-1 text-[10px] text-gray-600 border border-obsidian-400/40 rounded px-1">⌘K</span>
            </button>
            <button onClick={() => setShowCmd(true)} className="md:hidden p-2 rounded-lg text-gray-500 hover:text-white hover:bg-obsidian-600/60 transition-colors">
              <Search size={18} />
            </button>
            {isBanker ? (
              <button
              ...
              <button onClick={() => navigate('/reminders')} className="relative p-2 rounded-lg text-gold-400 hover:text-gold-300 hover:bg-obsidian-600/60 transition-colors">
                <Bell size={18} />
                {pendingCount > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 bg-red-500 rounded-full text-[9px] text-white flex items-center justify-center font-bold shadow-[0_0_8px_rgba(239,68,68,0.5)]">
                    {pendingCount}
            ...
            <button onClick={handleLogout} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-gold-400 hover:text-gold-300 hover:bg-obsidian-600/60 transition-colors text-sm">
              <LogOut size={15} />
        ...
        {pushStatus !== 'granted' && pushStatus !== 'unavailable' && (
          <div className="md:hidden flex items-center justify-between gap-3 px-4 py-2.5 bg-gold-500/10 border-b border-gold-500/20">
            ...
            <button onClick={requestPermission} className="shrink-0 text-xs font-semibold px-3 py-1.5 rounded-lg bg-gold-500 text-obsidian-950 active:bg-gold-600 transition-colors">
              Enable
            </button>
          </div>
        )}

        <div className="md:hidden flex justify-center items-end overflow-hidden transition-all duration-200 ease-out"
          style={{ height: isRefreshing ? 48 : pullDistance > 0 ? pullDistance : 0 }}
        >
          <div className="mb-2 w-8 h-8 rounded-full bg-obsidian-800 border border-gold-500/30 flex items-center justify-center shadow-gold-sm">
            {isRefreshing ? (
              <Loader2 size={15} className="text-gold-400 animate-spin" />
            ) : (
              <RefreshCw size={15} className="text-gold-400 transition-transform duration-100" style={{ transform: `rotate(${Math.min((pullDistance / 75) * 180, 180)}deg)` }} />
            )}
          </div>
        </div>
        ...
        <main
          ...
          style={{ paddingBottom: 'calc(6rem + env(safe-area-inset-bottom))', overscrollBehaviorY: 'none' }}
        ><Outlet /></main>
      </div>

      {isDirectorOrAdmin && <QuickActions onSearch={() => setShowCmd(true)} />}

      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 glass-header border-t border-gold-500/20 flex items-center justify-around px-1 shadow-card-lg"
      ...
      >
        {primaryNav.map(item => (
          <NavLink
            key={item.to}
            to={item.to}
            className={({ isActive }) => `flex flex-col items-center gap-0.5 px-2 py-2 rounded-xl transition-all ${isActive ? 'text-gold-300' : 'text-gray-400'}`}
          >
            {({ isActive }) => (
              <>
                <item.icon size={20} className={isActive ? 'drop-shadow-[0_0_8px_rgba(234,184,32,0.6)]' : ''} />
                <span className={`text-[10px] font-medium ${isActive ? 'text-gold-300' : ''}`}>{item.label}</span>
                {isActive && <span className="w-1.5 h-1.5 rounded-full bg-gold-300 shadow-[0_0_6px_rgba(234,184,32,0.7)]" />}
              </>
            )}
          </NavLink>
        ))}
        {moreNav.length > 0 && (
          <button onClick={() => setShowMore(true)} className={`flex flex-col items-center gap-0.5 px-2 py-2 rounded-xl transition-all ${showMore ? 'text-gold-300' : 'text-gray-400'}`}>
      ...
      <div className={`md:hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-40 transition-opacity duration-300 ${showMore ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setShowMore(false)} />
      <div
        className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-b from-obsidian-800 to-obsidian-900 border-t border-gold-500/10 rounded-t-3xl shadow-[0_-8px_40px_rgba(0,0,0,0.6)] transition-transform duration-300 ease-out ${showMore ? 'translate-y-0' : 'translate-y-full'}`}
        style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 24px)' }}
      >
        <div className="flex justify-center pt-3 pb-1"><div className="w-10 h-1 rounded-full bg-obsidian-400/50" /></div>
        <div className="flex items-center justify-between px-5 py-3 border-b border-obsidian-400/20">
          ...
          <button onClick={() => setShowMore(false)} className="w-7 h-7 flex items-center justify-center rounded-full bg-obsidian-700/60 text-gray-400 hover:text-white transition-colors">
            <X size={14} />
          </button>
        </div>
        <div className="grid grid-cols-3 gap-2.5 p-4">
          {moreNav.map(item => (
            <NavLink
              ...
              className={({ isActive }) => `flex flex-col items-center gap-2 p-3.5 rounded-2xl border transition-all active:scale-95 ${isActive ? 'bg-gradient-to-br from-gold-500/[0.18] to-transparent border-gold-400/40 text-gold-300 shadow-gold-sm' : 'bg-obsidian-700/50 border-obsidian-400/50 text-gray-400 hover:text-white hover:border-obsidian-300/40 hover:bg-obsidian-600/60'}`}
            >
              {({ isActive }) => (
                <>
      ...
      {toasts.length > 0 && createPortal(
        <div className="fixed bottom-24 md:bottom-6 right-4 flex flex-col gap-2 z-[600] pointer-events-none">
          {toasts.map(t => (
            <div key={t.id} className="pointer-events-auto flex w-80 bg-obsidian-800 border border-obsidian-400/30 rounded-2xl shadow-card-xl overflow-hidden animate-toast-in">
              ...
              <div className="flex-1 relative min-w-0">
                ...
                <button onClick={() => setToasts(p => p.filter(toast => toast.id !== t.id))} className="absolute top-2.5 right-2.5 p-1 rounded-lg text-gray-500 hover:text-gray-300 hover:bg-obsidian-600 transition-colors">
                  <X size={12} />
                </button>
              </div>
            </div>
      ...
      {isBanker && showNotifPanel && createPortal(
        <>
          ...
          <div className="fixed w-80 max-h-96 overflow-y-auto bg-obsidian-800 border border-obsidian-400/30 rounded-2xl shadow-2xl z-[499]" style={{ top: notifPanelTop, right: 16 }}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-obsidian-400/20 sticky top-0 bg-obsidian-800">
              ...
              {unreadNotifs.length > 0 && (
                <button
                  onClick={async () => {
                    await Promise.all(unreadNotifs.map(n => n.referenceId ? markNotificationsReadByRef(n.referenceId) : markNotificationReadById(n.id)));

...
const SALES_ROUTES = DIRECTOR_SALES.map((i) => i.to);
...
export default function Sidebar() {
          ...
          <div>
            {collapsed ? (
              <button
                ...
                className={`w-full flex justify-center px-2 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                  isSalesRouteActive ? 'text-gold-300 bg-gold-500/20' : 'text-white/60 hover:text-white hover:bg-white/[0.05]'
                }`}
              >
                <ShoppingBag size={18} className={isSalesRouteActive ? 'text-gold-300 drop-shadow-[0_0_6px_rgba(234,184,32,0.6)]' : 'text-gray-400'} />
              </button>
            ) : (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 group ${
                  isSalesRouteActive ? 'text-gold-300 bg-gradient-to-r from-gold-500/20 to-transparent' : 'text-white/60 hover:text-white hover:bg-white/[0.05]'
                }`}
              >
            ...
            {showSalesItems && (
              <div className="mt-1 space-y-0.5 border-l border-obsidian-400/60 ml-5">
                {renderItems(DIRECTOR_SALES, true)}
              </div>
            )}
          </div>

          <SidebarLabel collapsed={collapsed}>Management</SidebarLabel>
          {renderItems(DIRECTOR_MANAGEMENT)}
        </nav>
  ...
  const flatItems = getRoleNavItems(currentUser?.role);
      ...
      <div className={`flex items-center gap-3 min-w-0 ${collapsed ? 'justify-center' : ''}`}>
        ...
        {!collapsed && (
          <div className="min-w-0">
            ...
            <p className="text-[9px] mt-0.5 tracking-[0.18em] uppercase font-medium text-gold-300">Car Dealership</p>
          </div>
        )}
      </div>
      {onToggle && (
          ...
          {!collapsed && (
            <>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-400 text-[11px] capitalize mt-0.5">{currentUser?.position || currentUser?.role}</p>
              </div>
